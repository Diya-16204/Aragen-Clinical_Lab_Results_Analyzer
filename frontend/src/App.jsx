...
export default function App() {
  ...
  <section className="phase-one"><span>PHASE 1</span><b>Core Lab Analyzer</b><p>Input → deterministic classification → MCP + AI explanation</p></section><LabInput labs={labs} setLabs={setLabs} onAnalyze={submit} loading={loading} onLoadDemo={loadDemo} onReset={reset}/>{error && <div className="error">{error}</div>}{analysis && <><ClinicalOverview results={analysis.results} /><MCPActivity activity={analysis.agent_activity} /></>}<ResultsDisplay results={analysis?.results ?? null}/>{analysis && <><SeverityChart results={analysis.results} /><TrendHistory results={analysis.results} /></>}<footer>Reference ranges are demo defaults. Always interpret results in clinical context.</footer></>}</main>
}
